...
from database import close_db_pool, init_db_pool, SessionLocal, get_pool
...
async def check_liveness(frames: list[UploadFile] = File(...)):
    if len(frames) < 12 or len(frames) > 50:
        return JSONResponse(content={"passed": False, "message": "Invalid frame count"}, status_code=400)
    ...
    if len(usable_frames) < 12:
        return JSONResponse(content={"passed": False, "message": "Not enough usable frames"}, status_code=400)
    ...
    try:
        ...
        return JSONResponse(content={"passed": False, "message": "Processing failed"}, status_code=500)
...
async def mfa_setup(
    ...
    try:
        db.execute(
            text("INSERT INTO public.users (id, full_name, email, badge_number, security_clearance_level, is_active, created_at) VALUES (:id, :name, :email, :badge, 1, true, NOW())"),
            {
