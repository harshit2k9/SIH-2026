...
"""
"""
Document management router with strict security, streaming, and audit trails.
"""
import logging
import traceback
import uuid
...
from schemas import (
    DocumentUploadResponse, PaginatedDocumentsResponse, DocumentDownloadResponse,
    DocumentResponse, AuditLogResponse, DocumentMetadataUpdate, ShareDocumentRequest, DocumentVersionResponse,
)
from security.auth import AuthenticatedUser, verify_jwt
from security.rbac import require_upload_permission, require_case_access
...
async def upload_document(
    ...
    if doc_type_clean not in ALLOWED_DOC_TYPES:
        raise HTTPException(status.HTTP_422_UNPROCESSABLE_ENTITY, f"Invalid document_type. Must be one of: {', '.join(ALLOWED_DOC_TYPES)}")
    
    if not (1 <= confidentiality_level <= 5):
        raise HTTPException(status.HTTP_422_UNPROCESSABLE_ENTITY, "confidentiality_level must be between 1 and 5.")
    
    if not document_number or not document_number.strip():
        # FIXED: Use UUID fragment + timestamp for unpredictability
        document_number = f"DOC-{uuid.uuid4().hex[:8].upper()}-{int(datetime.now(timezone.utc).timestamp())}"
    else:
        document_number = document_number.strip()

    await require_upload_permission(user, case_id, pool)
    ...
    try:
        ...
        try:
            async with pool.acquire() as conn:
                async with conn.transaction():
                    ...
                    if not user_dept:
                        raise HTTPException(status.HTTP_400_BAD_REQUEST, "User has no department assigned.")

                    await conn.execute(
                        """
                        INSERT INTO documents (id, case_id, evidence_item_id, document_number, title,
                            document_type, confidentiality_level, current_version, created_by, created_at, is_locked)
                    ...
                    version_id = await conn.fetchval(
                        """
                        INSERT INTO document_versions (id, document_id, version_number, storage_uri, file_size_bytes,
                            file_mime_type, sha256_checksum, kms_key_id, uploaded_by, uploaded_at)
                        VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, NOW()) RETURNING id
                        """,
                        uuid.uuid4(), document_uuid, 1, storage_key, bytes_written, true_mime, file_hash, None, user.id,
                    )
                    
                    entry_hash = await write_audit_entry(
                        conn=conn, case_id=case_id, document_id=document_uuid, evidence_id=evidence_item_id,
                        actor_id=user.id, actor_department_id=user_dept, action="DOCUMENT_UPLOADED",
                        ip_address=extract_client_ip(request), user_agent=request.headers.get("User-Agent"),
                        details={"sha256": file_hash, "size_bytes": bytes_written, "mime": true_mime,
                                 "version_id": str(version_id), "version_number": 1, "storage_key": storage_key, "filename": safe_filename}
                    )
        ...
        return DocumentUploadResponse(document_id=document_uuid, sha256=file_hash, status="active", audit_entry_hash=entry_hash)
        ...
        _log_full_traceback(f"UPLOAD_FAILURE: {error_id}")
        _log_security_event("UPLOAD_FAILURE", user.id, f"error_id={error_id}")
        raise HTTPException(status.HTTP_500_INTERNAL_SERVER_ERROR, f"Upload failed. Reference ID: {error_id}")
    finally:
        if 'quarantine_path' in locals() and quarantine_path.exists():
            quarantine_path.unlink(missing_ok=True)

@router.get("/case/{case_id}", response_model=PaginatedDocumentsResponse)
async def list_case_documents(
    ...
    # FIXED: Added RBAC check
    await require_case_access(user, case_id, pool)
    
    offset = (page - 1) * limit
    params = [case_id]
    
    # FIXED: COUNT query now dynamically includes the doc_type filter
    count_query = "SELECT COUNT(*) FROM documents WHERE case_id = $1"
    if doc_type:
        count_query += " AND document_type = $2"
...
async def get_document_by_id(
    ...
    async with pool.acquire() as conn:
        # FIXED: Fetch case_id first to perform RBAC check
        doc = await conn.fetchrow("SELECT case_id FROM documents WHERE id = $1", document_id)
        if not doc:
            raise HTTPException(status.HTTP_404_NOT_FOUND, detail="Document not found")
            
        # FIXED: Added RBAC check
        await require_case_access(user, doc["case_id"], pool)
        ...
        record = await conn.fetchrow(
            ...
            SELECT d.id, d.case_id, d.title, d.document_type, d.document_number,
                   ...
                   d.current_version, d.confidentiality_level, d.is_locked
            FROM documents d
            JOIN document_versions dv ON d.id = dv.document_id AND dv.version_number = d.current_version
            WHERE d.id = $1
            """,
            document_id
        )
...
async def update_document_metadata(
    ...
    async with pool.acquire() as conn:
        doc = await conn.fetchrow("SELECT case_id, is_locked FROM documents WHERE id = $1", document_id)
        if not doc:
            raise HTTPException(status.HTTP_404_NOT_FOUND, detail="Document not found")
        ...
        if doc["is_locked"]:
            raise HTTPException(status.HTTP_403_FORBIDDEN, detail="Cannot update a locked document.")
        ...
        query = f"UPDATE documents SET {', '.join(update_fields)} WHERE id = ${param_idx} RETURNING case_id"
        
        async with conn.transaction():
            result = await conn.fetchrow(query, *params)
            user_dept = await conn.fetchval("SELECT department_id FROM user_departments WHERE user_id = $1 AND is_primary = TRUE", user.id)
            
            # FIXED: Updated to Pydantic V2 model_dump()
            await write_audit_entry(
                conn=conn, case_id=result["case_id"], document_id=document_id,
                actor_id=user.id, actor_department_id=user_dept, action="METADATA_UPDATED",
                ip_address=extract_client_ip(request), user_agent=request.headers.get("User-Agent"),
                details={"changes": updates.model_dump(exclude_none=True)}
            )
...
async def get_secure_download_url(
    ...
    async with pool.acquire() as conn:
        ...
        # FIXED: Added RBAC check
        await require_case_access(user, record["case_id"], pool)
        
        user_dept = await conn.fetchval("SELECT department_id FROM user_departments WHERE user_id = $1 AND is_primary = TRUE", user.id)
        
        await log_audit_event(
            conn=conn, case_id=record["case_id"], document_id=document_id,
            actor_id=user.id, actor_department_id=user_dept, action="PRESIGNED_URL_GENERATED",
            ip_address=extract_client_ip(request), user_agent=request.headers.get("User-Agent"),
            extra_details={"download_initiated": True}
        )
        
        presigned_url = await generate_presigned_download_url(record["s3_key"], file_name=record["file_name"])
        return DocumentDownloadResponse(
            document_id=record["id"], file_name=record["file_name"],
            presigned_url=presigned_url, expires_in_seconds=300
        )
