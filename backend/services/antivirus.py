...
async def scan_file(file_path: str) -> ScanResult:
    try:
        ...
        raise HTTPException(status.HTTP_503_SERVICE_UNAVAILABLE, "Security scanner is currently unavailable.")
    ...
    try:
        ...
        writer.write(struct.pack("!L", 0))  # EOF marker
        await writer.drain()
        ...
        raise HTTPException(status.HTTP_500_INTERNAL_SERVER_ERROR, "File scan failed.")
    finally:
        writer.close()
